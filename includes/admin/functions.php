...
function resize_images_in_posts__register_scripts_and_styles_admin_action() {
	$resize_images_in_posts                  = resize_images_in_posts();
	$includes_url                            = $resize_images_in_posts->includes_url;
	$version                                 = $resize_images_in_posts->version;
	
	// Admin Panel
	wp_register_script( 'resize-images-in-posts-admin-panel',                        $includes_url . 'admin/js/admin-menu.js',                         array(), $version, true ); 
	wp_register_style( 'resize-images-in-posts-admin-panel',                         $includes_url . 'admin/css/admin-default.css',                    array(), $version );
}

// Function that is used with AJAX to save all settings from the admin panel
function resize_images_in_posts__admin_save_action() {
	$json_success                  = array();
	$json_error                    = array();
	$function_exists               = false;

	if ( ! check_ajax_referer( 'wp_ajax_resize_images_in_posts__ajax', 'resize_images_in_posts__nonce', false ) ) {
		$json_error[] = __( 'Invalid nonce.', 'resize_images_in_posts' );
	}
	
	if ( ! current_user_can( 'edit_theme_options' ) ) {
		$json_error[] = __( "Current user can't change options.", 'resize_images_in_posts' );
	}
	...
	if ( empty( $json_error ) ) {
		...
		if ( ! empty( $_POST['function_id'] ) ) {
			if ( ! empty( $_POST['data'] ) ) {
				...
				$settings = apply_filters( 'resize_images_in_posts__add_menu_page_settings', array() );
				...
				$json_error[] = __( 'Data is empty.', 'resize_images_in_posts' );
			}
		} else {
			$json_error[] = __( 'Empty function ID value.', 'resize_images_in_posts' );
		}
		...
		if ( (bool) $function_exists ) {
			...
			// Filter the options
			$options = apply_filters( 'resize_images_in_posts__filter_admin_menu_options_' . $function_name, $function_options );
												...
												if ( isset( $tag['id'] ) ) {
													...
													if ( isset( $ajax_data[$tag_id] ) and ! $use_default_value ) {
														...
														if ( (bool) $array_upload ) {
															$array_value = resize_images_in_posts__get_option( $save_tag_id );
			...
			$json_error[] = __( 'Function ID does not exist.', 'resize_images_in_posts' );
		}
...
function resize_images_in_posts__function_call_admin_panel() {
	global $plugin_page;
	...
	if ( function_exists( $function_id ) ) {
		...
		// Filter the options
		$data = apply_filters( 'resize_images_in_posts__filter_admin_menu_options_' . $function_id, $function_options );
		...
		resize_images_in_posts__admin_panel( $function_id, $data );
	}
}

// This function is called to add admin menus in the WordPress left menu
function resize_images_in_posts__admin_menu_action() {
	$settings = apply_filters( 'resize_images_in_posts__add_menu_page_settings', array() );
	...
	if ( ! empty ( $settings ) ) {
		foreach ( (array) $settings as $settings_id => $wpi ) {
			if ( isset( $wpi['page_title'], $wpi['menu_title'], $wpi['capability'], $wpi['menu_slug'] ) ) {
				...
				add_action( 'load-' . $add_plugins_page, 'resize_images_in_posts__admin_add_help_and_sidebar_action' );				
			}
		}
	}
}
...
function resize_images_in_posts__admin_add_help_and_sidebar_action() {
	global $plugin_page;
	...
	$resize_images_in_posts__admin_panel_help = resize_images_in_posts__admin_panel_help();
	
	// Default sidebar text
	$sidebar_tab_help = resize_images_in_posts__sidebar_tab_help();
	
	if ( ! empty ( $settings ) ) {
		foreach ( (array) $settings as $sub_settings ) {
		
			// Filter the admin_panel_help
			$resize_images_in_posts__admin_panel_help_filtered = apply_filters( 'resize_images_in_posts__filter_admin_panel_help', $resize_images_in_posts__admin_panel_help, $plugin_page );
			
			// Filter the sidebar help tab text
			$sidebar_tab_help_filtered = apply_filters( 'resize_images_in_posts__filter_admin_panel_help', $sidebar_tab_help, $plugin_page );
			...
			if ( (string) $sub_settings['menu_slug'] == (string) $plugin_page ) {
				...
				if ( ! empty( $sub_settings['content'] ) and (string) $sub_settings['content'] == 'data' ) {
					if ( ! empty( $sub_settings['help'] ) ) {
						$admin_panel_help = array_merge( $resize_images_in_posts__admin_panel_help_filtered, $sub_settings['help'] );
					} else {
						$admin_panel_help = $resize_images_in_posts__admin_panel_help_filtered;
					}
...
function resize_images_in_posts__enqueue_style_scripts_action() {
	global $plugin_page;
...
function resize_images_in_posts__admin_panel_help() {
	$admin_panel_help = array(
		'id_1'   => array(
			'title'      => __( 'About Admin Panel', 'resize_images_in_posts' ),
			'content'    => '<p>' . __( 'Admin Panel will help you to change the settings of the theme.', 'resize_images_in_posts' ) . '</p>' . 
			'<p>' . __( 'To save the settings after change - click "Apply Changes".', 'resize_images_in_posts' ) . '</p>' .
			'<p>' . __( 'To reset all settings click "Default Values To All" button and save.', 'resize_images_in_posts' ) . '</p>'
		)
	);
...
function resize_images_in_posts__sidebar_tab_help() {
	$sidebar_tab_help = '<p><strong>' . __( 'For more information:', 'resize_images_in_posts' ) . '</strong></p>' .
		'<p>' . __( '<a href="https://wordpress.org/plugins/resize-images-in-posts/" target="_blank">' . __( 'Documentation', 'resize_images_in_posts' ) . '</a>' ) . '</p>' .
		'<p>' . __( '<a href="https://wordpress.org/support/plugin/resize-images-in-posts" target="_blank">' . __( 'Support Forum', 'resize_images_in_posts' ) . '</a>' ) . '</p>';
	
	return $sidebar_tab_help;
}


function resize_images_in_posts__settings_site_info() {
	$resize_images_in_posts = resize_images_in_posts();
	$wp_get_theme = wp_get_theme();
	
	$site_info = '';
	
	$site_info .= __( 'Theme:', 'resize_images_in_posts' ) . ' ' . $wp_get_theme->get( 'Name' ) . "\n";

	$site_info .= __( 'Versions:', 'resize_images_in_posts' ) . "\n";
	$site_info .= __( '- Plugin:', 'resize_images_in_posts' ) . ' ' .  $resize_images_in_posts->version . "\n";
